...
可插拔加密算法实现，支持国际算法、国密算法和 NRCS 自定义算法。

## 特性

- **哈希算法**: SHA-256 (默认) 或 SM3
- **签名算法**: Ed25519 (默认)
  - ⚠️ Curve25519 (NRCS 兼容) - 重构中，暂时不可用
  - ⚠️ SM2 - 待修复
- **对称加密**: SM4-GCM (默认)
- **助记词**: NRCS 标准 12 词助记词系统

## Curve25519 状态

⚠️ **Curve25519 签名算法正在重构中，暂时不可用。**

NRCS 使用的是自定义的 Curve25519 签名算法，与标准 Ed25519 不同。由于 Rust 借用检查问题，该算法正在重构中。

详细的重构计划请参见：[Curve25519 重构计划](docs/curve25519_refactoring.md)
