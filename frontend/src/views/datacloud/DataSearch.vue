<template>
  <div class="page-container">
    <div class="page-header">
      <h2 class="page-title"><el-icon><Search /></el-icon> 数据云搜索</h2>
...
async function refreshData() {
  ...
  try {
    const result = await nrcsApi.searchTaggedData('', (currentPage.value-1)*20, currentPage.value*20-1); items.value = (result.data || []).map((d: any) => ({ id: d.transaction, name: d.name, timestamp: d.timestamp })); total.value = items.value.length;
  } catch (error) {
