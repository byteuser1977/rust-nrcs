<template>
  <div class="page-container">
    <div class="page-header">
      <h2 class="page-title"><el-icon><Money /></el-icon> 积分</h2>
...
async function refreshData() {
  ...
  try {
    const result = await nrcsApi.getAllCurrencies((currentPage.value-1)*20, currentPage.value*20-1); items.value = (result.currencies || []).map((c: any) => ({ id: c.currency, name: c.code })); total.value = items.value.length;
  } catch (error) {
