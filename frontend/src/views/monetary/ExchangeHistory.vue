<template>
  <div class="page-container">
    <div class="page-header">
      <h2 class="page-title"><el-icon><Timer /></el-icon> 交易历史</h2>
...
async function refreshData() {
  ...
  try {
    const result = await nrcsApi.getAllExchanges((currentPage.value-1)*20, currentPage.value*20-1); items.value = (result.exchanges || []).map((e: any) => ({ id: e.exchange, name: e.currency, timestamp: e.timestamp })); total.value = items.value.length;
  } catch (error) {
