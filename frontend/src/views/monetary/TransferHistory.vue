<template>
  <div class="page-container">
    <div class="page-header">
      <h2 class="page-title"><el-icon><Sort /></el-icon> 转移历史</h2>
...
async function refreshData() {
  ...
  try {
    const account = localStorage.getItem('nrcs_account_rs'); if (account) { const result = await nrcsApi.getAccountCurrencies(account); items.value = result.currencyBalances || []; total.value = items.value.length; }
  } catch (error) {
