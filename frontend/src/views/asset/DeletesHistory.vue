<template>
  <div class="page-container">
    <div class="page-header">
      <h2 class="page-title"><el-icon><Delete /></el-icon> 删除历史</h2>
...
async function refreshData() {
  ...
  try {
    const result = await nrcsApi.getAllAssets((currentPage.value-1)*20, currentPage.value*20-1); items.value = (result.assets || []).map((a: any) => ({ id: a.asset, name: a.name })); total.value = items.value.length;
  } catch (error) {
