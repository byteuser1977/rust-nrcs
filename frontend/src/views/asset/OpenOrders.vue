<template>
  <div class="page-container">
    <div class="page-header">
      <h2 class="page-title"><el-icon><Document /></el-icon> 开放订单</h2>
...
async function refreshData() {
  ...
  try {
    const account = localStorage.getItem('nrcs_account_rs'); if (account) { const [asks, bids] = await Promise.all([nrcsApi.getAccountCurrentAskOrders(account), nrcsApi.getAccountCurrentBidOrders(account)]); items.value = [...(asks.askOrders || []), ...(bids.bidOrders || [])]; total.value = items.value.length; }
  } catch (error) {
