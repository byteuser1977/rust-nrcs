...
async function refreshData() {
  ...
  try {
    const result = await nrcsApi.getTrades(undefined, (currentPage.value-1)*20, currentPage.value*20-1); items.value = (result.trades || []).map((t: any) => ({ id: t.trade, name: t.asset, timestamp: t.timestamp })); total.value = items.value.length;
  } catch (error) {
