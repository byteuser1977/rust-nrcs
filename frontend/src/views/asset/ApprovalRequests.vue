  ...
  <div class="page-container">
    ...
    <el-card shadow="hover" v-loading="loading">
      <el-table :data="items" stripe style="width:100%" :empty-text="t('common.noData')">
        ...
        <el-table-column prop="phasingFinishHeight" :label="t('asset.finishHeight')" width="110" align="right" />
        <el-table-column :label="t('asset.votes')" width="100" align="center">
          <template #default="{ row }">{{ row.phasingResult || '-' }}</template>
        </el-table-column>
...
async function refreshData() {
  ...
  try {
    ...
    const result = await nrcsApi.getAccountPhasedTransactions(accountId, (page.value - 1) * 20, page.value * 20 - 1)
