<template>
  <div class="page-container">
    <div class="page-header">
      <h2 class="page-title"><el-icon><ChatLineRound /></el-icon> 聊天</h2>
...
async function refreshData() {
  ...
  try {
    const account = localStorage.getItem('nrcs_account_rs'); if (account) { const result = await nrcsApi.getAccountMessages(account, (currentPage.value-1)*20, currentPage.value*20-1); items.value = (result.messages || []).map((m: any) => ({ id: m.transaction, name: m.attachment?.message, timestamp: m.timestamp })); total.value = items.value.length; }
  } catch (error) {
