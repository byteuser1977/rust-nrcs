<template>
  <div class="page-container">
    <div class="page-header">
      <h2 class="page-title"><el-icon><Sell /></el-icon> 我的出售产品</h2>
...
async function refreshData() {
  ...
  try {
    const account = localStorage.getItem('nrcs_account_rs'); if (account) { const result = await nrcsApi.getDGSGoods(account, (currentPage.value-1)*20, currentPage.value*20-1); items.value = (result.goods || []).map((g: any) => ({ id: g.goods, name: g.name, timestamp: g.timestamp })); total.value = items.value.length; }
  } catch (error) {
