<template>
  <div class="page-container">
    <div class="page-header">
      <h2 class="page-title"><el-icon><Clock /></el-icon> 待处理订单</h2>
...
async function refreshData() {
  ...
  try {
    const account = localStorage.getItem('nrcs_account_rs'); if (account) { const result = await nrcsApi.getDGSPendingPurchases(account, (currentPage.value-1)*20, currentPage.value*20-1); items.value = (result.purchases || []).map((p: any) => ({ id: p.purchase, name: p.name, timestamp: p.timestamp })); total.value = items.value.length; }
  } catch (error) {
