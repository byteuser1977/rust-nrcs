<template>
  <div class="page-container">
    <div class="page-header">
      <h2 class="page-title"><el-icon><Grid /></el-icon> 区块</h2>
...
async function refreshData() {
  ...
  try {
    const result = await nrcsApi.getBlocks((currentPage.value-1)*20, currentPage.value*20-1); items.value = (result.blocks || []).map((b: any) => ({ id: b.block, name: b.generatorRS, timestamp: b.timestamp })); total.value = items.value.length;
  } catch (error) {
