<template>
  <div class="page-container">
    <div class="page-header">
      <h2 class="page-title"><el-icon><Cpu /></el-icon> 生成者</h2>
...
async function refreshData() {
  ...
  try {
    const result = await nrcsApi.getForging(); items.value = (result.generators || []).map((g: any) => ({ id: g.account, name: g.accountRS })); total.value = items.value.length;
  } catch (error) {
