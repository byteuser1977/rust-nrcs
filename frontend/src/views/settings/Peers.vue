<template>
  <div class="page-container">
    <div class="page-header">
      <h2 class="page-title"><el-icon><Connection /></el-icon> 节点</h2>
...
async function refreshData() {
  ...
  try {
    const result = await nrcsApi.getPeers(undefined, true); items.value = (result.peers || []).map((p: any) => ({ id: p.address, name: p.application || p.announcedAddress })); total.value = items.value.length;
  } catch (error) {
