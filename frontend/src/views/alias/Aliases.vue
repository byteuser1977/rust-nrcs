<template>
  <div class="page-container">
    <div class="page-header">
      <h2 class="page-title"><el-icon><Bookmark /></el-icon> 别名</h2>
...
async function refreshData() {
  ...
  try {
    const account = localStorage.getItem('nrcs_account_rs'); if (account) { const result = await nrcsApi.getAliases(account, (currentPage.value-1)*20, currentPage.value*20-1); items.value = (result.aliases || []).map((a: any) => ({ id: a.alias, name: a.aliasName, timestamp: a.timestamp })); total.value = items.value.length; }
  } catch (error) {
