<template>
  <div class="page-container">
    <div class="page-header">
      <h2 class="page-title"><el-icon><Setting /></el-icon> 账户属性</h2>
...
async function refreshData() {
  ...
  try {
    const account = localStorage.getItem('nrcs_account_rs') || localStorage.getItem('nrcs_account_id'); if (account) { const result = await nrcsApi.getAccountProperties(account); items.value = (result.properties || []).map((p: any) => ({ id: p.property, name: p.value })); total.value = items.value.length; }
  } catch (error) {
