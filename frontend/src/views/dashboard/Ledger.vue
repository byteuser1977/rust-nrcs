<template>
  <div class="page-container">
    <div class="page-header">
      <h2 class="page-title"><el-icon><Notebook /></el-icon> 账户总账</h2>
...
async function refreshData() {
  ...
  try {
    const account = localStorage.getItem('nrcs_account_rs') || localStorage.getItem('nrcs_account_id'); if (account) { const result = await nrcsApi.getAccountLedger(account, (currentPage.value-1)*20, currentPage.value*20-1); items.value = result.entries || []; total.value = items.value.length; }
  } catch (error) {
