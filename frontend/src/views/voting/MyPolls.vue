<template>
  <div class="page-container">
    <div class="page-header">
      <h2 class="page-title"><el-icon><EditPen /></el-icon> 我创建的投票</h2>
...
async function refreshData() {
  ...
  try {
    const account = localStorage.getItem('nrcs_account_rs'); if (account) { const result = await nrcsApi.getBlockchainTransactions(account, (currentPage.value-1)*20, currentPage.value*20-1, 7, 0); items.value = (result.transactions || []).map((tx: any) => ({ id: tx.transaction, timestamp: tx.timestamp })); total.value = items.value.length; }
  } catch (error) {
