<template>
  <el-dialog v-model="visible" :title="t('marketplace.refund')" width="500px" :close-on-click-modal="false" destroy-on-close class="nrcs-modal" @close="handleClose">
    ...
    <el-form ref="formRef" :model="form" label-position="top">
      <el-form-item :label="t('marketplace.refundAmount')">
        <el-input v-model="form.refundNQT" type="number" placeholder="0.00" clearable><template #append>NRC</template></el-input>
      </el-form-item>
      <el-row :gutter="16">
    ...
    <template #footer>
      ...
      <el-button type="primary" :loading="loading" @click="handleSubmit">{{ t('marketplace.refund') }}</el-button>
    </template>
  </el-dialog>
</template>
...
const form = reactive({ refundNQT: '', feeNQT: '1', deadline: '1440', secretPhrase: '' })
...
async function handleSubmit() {
  ...
  await formRef.value.validate(async (valid) => {
    ...
    try {
      ...
      if (form.refundNQT) data.refundNQT = String(Math.round(Number(form.refundNQT) * 1e8))
      await nrcsApi.dgsRefund(data)
      ElMessage.success(t('marketplace.refundSuccess'))
      emit('success'); handleClose()
    } catch (err: any) { ElMessage.error(err?.message || t('marketplace.refundError')) }
