<template>
  <el-dialog v-model="visible" :title="t('account.setInfo')" width="520px" :close-on-click-modal="false" destroy-on-close class="nrcs-modal" @close="handleClose">
    ...
    <template #footer>
      ...
      <el-button type="primary" :loading="loading" @click="handleSubmit">{{ t('account.setInfo') }}</el-button>
    </template>
  </el-dialog>
</template>
...
const form = reactive({ name: '', description: '', feeNQT: '1', deadline: '1440', secretPhrase: '' })

const rules = computed<FormRules>(() => ({
  name: [{ required: true, message: t('account.nameRequired'), trigger: 'blur' }],
  description: [{ required: true, message: t('account.descriptionRequired'), trigger: 'blur' }],
...
async function handleSubmit() {
  ...
  await formRef.value.validate(async (valid) => {
    ...
    loading.value = true
    try {
      await nrcsApi.setAccountInfo({ secretPhrase: form.secretPhrase, name: form.name, description: form.description, feeNQT: String(Math.round(Number(form.feeNQT) * 1e8)), deadline: Number(form.deadline) })
      ElMessage.success(t('account.setInfoSuccess'))
      emit('success'); handleClose()
    } catch (err: any) { ElMessage.error(err?.message || t('account.setInfoError')) }
