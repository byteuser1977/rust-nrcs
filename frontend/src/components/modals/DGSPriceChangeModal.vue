<template>
  <el-dialog v-model="visible" :title="t('marketplace.changePrice')" width="480px" :close-on-click-modal="false" destroy-on-close class="nrcs-modal" @close="handleClose">
    ...
    <template #footer>
      ...
      <el-button type="primary" :loading="loading" @click="handleSubmit">{{ t('marketplace.changePrice') }}</el-button>
    </template>
  </el-dialog>
</template>
...
const form = reactive({ priceNQT: '', feeNQT: '1', deadline: '1440', secretPhrase: '' })
...
async function handleSubmit() {
  ...
  await formRef.value.validate(async (valid) => {
    ...
    loading.value = true
    try {
      await nrcsApi.dgsPriceChange({ secretPhrase: form.secretPhrase, goods: props.goods?.goods, priceNQT: String(Math.round(Number(form.priceNQT) * 1e8)), feeNQT: String(Math.round(Number(form.feeNQT) * 1e8)), deadline: Number(form.deadline) })
      ElMessage.success(t('marketplace.priceChangeSuccess'))
      emit('success'); handleClose()
    } catch (err: any) { ElMessage.error(err?.message || t('marketplace.priceChangeError')) }
