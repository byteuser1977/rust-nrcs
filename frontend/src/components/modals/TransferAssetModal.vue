<template>
  <el-dialog v-model="visible" :title="t('asset.transferAsset')" width="520px" :close-on-click-modal="false" destroy-on-close class="nrcs-modal" @close="handleClose">
    <el-form ref="formRef" :model="form" :rules="rules" label-position="top">
      <el-form-item :label="t('asset.asset')" prop="asset">
        <el-input v-model="form.asset" :placeholder="t('asset.assetIdPlaceholder')" clearable />
      </el-form-item>
...
const form = reactive({ asset: '', recipient: '', quantityQNT: '', feeNQT: '1', deadline: '1440', secretPhrase: '' })

const rules = computed<FormRules>(() => ({
  asset: [{ required: true, message: t('asset.assetRequired'), trigger: 'blur' }],
...
async function handleSubmit() {
  ...
  await formRef.value.validate(async (valid) => {
    ...
    loading.value = true
    try {
      await nrcsApi.transferAsset({ secretPhrase: form.secretPhrase, asset: form.asset, recipient: form.recipient, quantityQNT: form.quantityQNT, feeNQT: String(Math.round(Number(form.feeNQT) * 1e8)), deadline: Number(form.deadline) })
      ElMessage.success(t('asset.transferSuccess'))
      emit('success'); handleClose()
    } catch (err: any) { ElMessage.error(err?.message || t('asset.transferError')) }
